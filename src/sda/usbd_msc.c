...
bool        USBD_MSC_MediaReadyEx = false; /* Previous state of Media ready */
...
__attribute__((weak)) void usbd_msc_init       (void)                                      {};
__attribute__((weak)) void usbd_msc_read_sect  (uint32_t block, uint8_t *buf, uint32_t num_of_blocks) {(void)block; (void)buf; (void)num_of_blocks;};
__attribute__((weak)) void usbd_msc_write_sect (uint32_t block, uint8_t *buf, uint32_t num_of_blocks) {(void)block; (void)buf; (void)num_of_blocks;};
__attribute__((weak)) void usbd_msc_start_stop (bool start)                            {(void)start;};
...
void USBD_MSC_SetStallEP (uint32_t EPNum) {     /* set EP halt status according stall status */
...
void USBD_MSC_ClrStallEP (uint32_t EPNum) {     /* clear EP halt status according stall status */
  uint32_t n,m;
  ...
  if ((n == (usbd_msc_ep_bulkin | 0x80)) && ((USBD_EndPointHalt & m) != 0)) {
    ...
    if (USBD_MSC_CSW.dSignature == MSC_CSW_Signature) {
      USBD_WriteEP((usbd_msc_ep_bulkin | 0x80), (uint8_t *)&USBD_MSC_CSW, sizeof(USBD_MSC_CSW));
    }
  }
}
...
bool USBD_MSC_RWSetup (void) {
  ...
  switch (USBD_MSC_CBW.CB[0]) {
    case SCSI_WRITE10:
    case SCSI_VERIFY10:
      if (!USBD_MSC_CheckMedia()) return (false);
    case SCSI_READ10:
...
void USBD_MSC_Inquiry (void) {
  ...
  ptr_str = (uint8_t *)usbd_msc_inquiry_data;
...
void USBD_MSC_GetCBW (void) {
  ...
  for (n = 0; n < BulkLen; n++) {
    *((uint8_t *)&USBD_MSC_CBW + n) = USBD_MSC_BulkBuf[n];
  }
...
void USBD_MSC_EP_BULKIN_Event (uint32_t event) {
  USBD_MSC_BulkIn();
}
...
void USBD_MSC_EP_BULKOUT_Event (uint32_t event) {
...
void USBD_MSC_EP_BULK_Event (uint32_t event) {
