...
static int32_t nb_sector;
static int32_t current_sector;
...
static int32_t start_sector;
static int32_t theoretical_start_sector = 7;
static uint8_t msc_event_timeout;
static uint8_t good_file;
static uint8_t program_page_error;
static uint8_t maybe_erase;
static int32_t previous_sector;
static int32_t begin_sector;
...
static const char * reason_array[] = {
    "SWD ERROR",
    "BAD EXTENSION FILE",
...
static thread_t * msc_valid_file_timeout_task_id;
...
void msc_valid_file_timeout_task(void) {
#if 0
    uint32_t flags = 0;
    msg_t res;
    uint32_t start_timeout_time = 0, time_now = 0;
    uint8_t timer_started = 0;
    msc_valid_file_timeout_task_id = chThdGetSelfX();
    while (1) {
...
void usbd_msc_read_sect (int32_t block, uint8_t *buf, uint32_t num_of_blocks) {
...
void usbd_msc_write_sect (int32_t block, uint8_t *buf, uint32_t num_of_blocks) {
    int idx_size = 0;
